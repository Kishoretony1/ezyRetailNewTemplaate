...
export class PaymentOptionsComponent implements OnInit, AfterViewInit
{
  ...
  onAddSubmit(form : any){
    this.submitted = true;
     ...
     this.apiService.post('ezyCart/paymentoption' , this.saveAddForm.value).subscribe((obj  : any) => {
      const dialogRef = this.dialog.open(DialogComponent, {
        disableClose: false,
        width: "auto",
        data: {
  ...
  onSubmit(form : any){
    ...
    let formValue = this.saveForm.value;
     ...
     this.apiService.put('ezyCart/paymentoption/'+'?id='+this.value.id , formValue).subscribe((obj  : any)   => {
  ...
  getPaymentOptionsList() {
    let id_user : id_user = this.apiService.getlocalstorageId();
    this.apiService.get('ezyCart/paymentoption').subscribe((obj: any) => {
      if (obj) {
        this.data = obj;
