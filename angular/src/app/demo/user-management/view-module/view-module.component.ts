...
export class ViewModuleComponent implements OnInit, AfterViewInit
 {
  displayedColumns: string[] = ['sno', 'moduleName','status', 'action'];
  ...
  constructor(private apiService: ApiService,private router : Router, private formBuilder: FormBuilder, private dialog: MatDialog,private http:HttpClient) {
    this.saveForm = this.formBuilder.group({
      moduleName: [{ value: '',  }, Validators.required],
  ...
  onAddSubmit(form : any){
    this.submitted = true;
     ...
     this.apiService.post('cms/module' , this.saveAddForm.value).subscribe((obj  : any) => {
      const dialogRef = this.dialog.open(DialogComponent, {
        disableClose: false,
        width: "auto",
        data: {
  ...
  onSubmit(form : any){
    ...
    let id_user: id_user = this.apiService.getlocalstorageId();
     ...
     this.apiService.put('cms/module/'+this.value.id, this.saveForm.value).subscribe((obj  : any)   => {
  ...
  getModuleList() {
    let id_user : id_user = this.apiService.getlocalstorageId();
    this.apiService.get('cms/module').subscribe((obj: any) => {
      if (obj) {
        this.data = obj.data;
