...
{
  displayedColumns: string[] = ['sno', 'roleName','roleAccess', 'action'];
...
//
    ...
    this.submitted = true;
     if(form.invalid){
      ...
      const roleAccessObject = {
       ...
       this.apiService.post('cms/roles' ,
      {
         roleName: this.saveForm.value.roleName,
         roleAccess: this.roleAccessObject,
        }).subscribe((obj  : any) => {
      const dialogRef = this.dialog.open(DialogComponent, {
        disableClose: false,
        width: "auto",
  ...
  stringifyRoleAccess(roleAccess: any): string {
  ...
  getFeatureList() {
    let id_user : id_user = this.apiService.getlocalstorageId();
    this.apiService.get('cms/roles').subscribe((obj: any) => {
      if (obj) {
        this.data = obj.data;
