...
@Component({
  selector: 'app-by-transaction-lite',
  templateUrl: './by-transaction-lite.component.html',
  styleUrls: ['./by-transaction-lite.component.scss']
})
export class ByTransactionLiteComponent implements OnInit
{
  ...
  submitted = false;
  saveForm!: FormGroup;
  displayedColumns: string[] = ['invoiceNum', 'transactionDate', 'paymenttype', 'paymentRef', 'totalItems', 'subTotal', 'tax', 'promodiscount', 'discount' ,'grandTotal'
 // , 'privilege'
];
  ...
  getDashboardDataForDateRange(startDate: string, endDate: string) {
    ...
    this.apiService.get('reports/sales/transactions?' +queryParams+"&lightVersionCarts=1"
      )
      .subscribe((obj: any) => {
        if (obj) {
          console.log(obj);
  ...
  getTicketList(days: string) {
    ...
    this.apiService.get('reports/sales/transactions?' + queryParams+"&lightVersionCarts=1").subscribe(
        (obj: any) => {
            if (obj) {
                console.log(obj);
                this.data = obj.data;
  ...
  rangeSelection(event: any) {
    ...
    currentData() {
      ...
      transactionDetails(row : any){
        ...
        this.router.navigate(['/SalesReport/transactiondetails']);
      }
