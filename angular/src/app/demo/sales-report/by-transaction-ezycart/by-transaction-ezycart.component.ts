...
@Component({
  selector: 'app-by-transaction-ezycart',
  templateUrl: './by-transaction-ezycart.component.html',
  styleUrls: ['./by-transaction-ezycart.component.scss']
})
export class ByTransactionEzycartComponent implements OnInit
 {
 ...
 // , 'privilege'
  ...
  constructor(public auth : AuthService, private formBuilder: FormBuilder, private dialog: MatDialog, private router: Router, private apiService: ApiService,public location : Location) {
    ...
    getDashboardDataForDateRange(startDate: string, endDate: string) {
      ...
      this.apiService.get('reports/sales/transactions?' +queryParams
        )
        .subscribe((obj: any) => {
          if (obj) {
            console.log(obj);
