...
@Component({
  ...
  selector: 'app-view-zones-beacons',
  templateUrl: './view-zones-beacons.component.html',
  styleUrls: ['./view-zones-beacons.component.scss'],
...
export  class ViewZonesBeaconsComponent implements OnInit, AfterViewInit
{


  displayedColumns: string[] =['zoneName', 'zoneType', 'beacons', 'action'];
  ...
  constructor(private apiService: ApiService,private router : Router, private formBuilder: FormBuilder, private dialog: MatDialog,private http:HttpClient) {
    this.saveForm = this.formBuilder.group({
      zoneName: [{ value: '',  }, Validators.required],
      zoneType: [{ value: '',  }, Validators.required],
  ...
  onAddSubmit(form : any){
    this.submitted = true;
     ...
     this.apiService.post('cms/zone' , this.saveAddForm.value).subscribe((obj  : any) => {
      const dialogRef = this.dialog.open(DialogComponent, {
        disableClose: false,
        width: "auto",
        data: {
  ...
  onSubmit(form : any){
    ...
    let id_user: id_user = this.apiService.getlocalstorageId();
     ...
     this.apiService.put('cms/zone/'+this.value.zoneId, this.saveForm.value).subscribe((obj  : any)   => {
      ...
      const dialogRef = this.dialog.open(DialogComponent, {
        ...
        data: {
          type : 'success',
          message: this.saveForm.value.zoneName+" updated successfully",
          btnClose: "OK"
        }
      });
  ...
  getZoneList() {
    let id_user : id_user = this.apiService.getlocalstorageId();
    this.apiService.get('cms/zones?merchantId='+id_user.merchantId+'&outletId='+id_user.outletId).subscribe((obj: any) => {
      if (obj) {
        // obj = JSON.parse(obj);
        this.data = obj.data
  ...
  beaconsList(row : any){
    localStorage.setItem('zoneId', row.zoneId);
     this.router.navigate(['dashboard/zones-beacons/view_beacons']);
   }
  closeChatBox() {
  ...
  deleteZones(row : any)
  {
    ...
    dialogRef.afterClosed().subscribe(result=>{
      if(result == 'yes'){
        let id_user : id_user = this.apiService.getlocalstorageId();
    this.apiService.delete('cms/zone/'+row.zoneId).subscribe((obj: any) =>{
          const dialogRef = this.dialog.open(DialogComponent, {
            disableClose: false,
            width: "300px",
            data: {
