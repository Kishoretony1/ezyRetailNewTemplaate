...
@Component({
  selector: 'app-ezy-cart',
  templateUrl: './ezy-cart.component.html',
  styleUrls: ['./ezy-cart.component.scss'],
  animations: [
...
export class EzyCartComponent implements OnInit, AfterViewInit

{


  displayedColumns: string[] = ['deviceId', 'trollyNo', 'appVersion', 'cartInfo.createdAt', 'deviceStatus', 'action'];
  ...
  getEzyList() {
    let id_user : id_user = this.apiService.getlocalstorageId();
    this.apiService.get('ezyRetail/devices?merchatId='+id_user.merchantId+'&outletId='+id_user.outletId+'&deviceMode=1').subscribe((obj: any) => {
      if (obj) {
        // console.log(obj);
