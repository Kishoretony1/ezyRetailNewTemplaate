...
@Component({
  selector: 'app-ezy-cart-lite',
  templateUrl: './ezy-cart-lite.component.html',
  styleUrls: ['./ezy-cart-lite.component.scss'],
  animations: [
...
export class EzyCartLiteComponent implements OnInit, AfterViewInit
{
  displayedColumns: string[] = ['deviceId',  'appVersion', 'cartInfo.createdAt', 'deviceStatus', 'action'];



  dataSource!: MatTableDataSource<beacon_List>;
  ...
  getEzyList() {
    let id_user : id_user = this.apiService.getlocalstorageId();
    this.apiService.get('ezyRetail/devices?merchatId='+id_user.merchantId+'&outletId='+id_user.outletId+'&deviceMode=2').subscribe((obj: any) => {
      if (obj) {
        // console.log(obj);
  ...
  deleteEzyList(row : any)
  {
    ...
    dialogRef.afterClosed().subscribe(result=>{
      if(result == 'yes'){
        let id_user : id_user = this.apiService.getlocalstorageId();
    this.apiService.delete('ezyRetail/devices/'+row.id).subscribe((obj: any) =>{
          const dialogRef = this.dialog.open(DialogComponent, {
            disableClose: false,
            width: "300px",
            data: {
              type : 'success',
              message: 'Ezycart deleted Successfully',
              btnClose: "Ok"
            }
          });
  ...
  viewTrollyDetails(row : any){
    localStorage.setItem('trollyNo', row.trollyNo);
    // localStorage.setItem('qrCode', row.qrCode);
    // localStorage.setItem('page', 'transactions');
    this.router.navigate(['dashboard/devices/trollyNumberDetails']);
    // this.router.navigate(['/devices/trollyNumberDetails']);
  }
