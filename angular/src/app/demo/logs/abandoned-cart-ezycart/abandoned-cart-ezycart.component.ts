...
@Component({
  selector: 'app-abandoned-cart-ezycart',
  templateUrl: './abandoned-cart-ezycart.component.html',
  styleUrls: ['./abandoned-cart-ezycart.component.scss']
})
export class AbandonedCartEzycartComponent  implements OnInit
{
  ...
  constructor(public auth : AuthService, private formBuilder: FormBuilder, private dialog: MatDialog, private router: Router, private apiService: ApiService,public location : Location) {
    ...
    getDashboardDataForDateRange(startDate: string, endDate: string) {
      ...
      this.apiService.get("reports/sales/transactions?"+ queryParams + "&showAbundant=1"
        )
        .subscribe((obj: any) => {
          if (obj) {
            console.log(obj);
